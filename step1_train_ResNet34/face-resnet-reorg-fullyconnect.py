...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        # features = tf.identity(logits, name = 'feat')
        numclasses = 20000
        y = FullyConnected('y', logits,  numclasses, nl=tf.identity)
        # newclasses = 1000
        # ynew = FullyConnected('ynew', logits, newclasses, nl=tf.identity)
        # W = tf.Variable(tf.zeros([512, numclasses]), name='classifier_w')
	# b = tf.Variable(tf.zeros([numclasses]), name='classifier_b')
        # y = tf.matmul(logits, W) + b
        # W_new = tf.Variable(tf.zeros([512, newclasses]), name='newclassier_w')
        # b_new = tf.Variable(tf.zeros(newclasses), name='newclassier_b')
        # y_new = tf.matmul(logits, W_new) + b_new
        # y_all = tf.concat([y, ynew], 1, name='yconcat')
        # self.var_trainable = [W, b]
        # self.var_trainable = [W_new, b_new]
        # self.var_trainable = [tensor_y_w, tensor_y_b]
        # self.var_trainable = None
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=label)
        ...
        wrong = prediction_incorrect(y, label, 1, name='wrong-top1')
        # wrong = prediction_incorrect(logits, label, 1, name='wrong-top1')
        add_moving_summary(tf.reduce_mean(wrong, name='train-error-top1'))

        # wrong = prediction_incorrect(y_all, label, 5, name='wrong-top5')
        wrong = prediction_incorrect(y, label, 5, name='wrong-top5')
        # wrong = prediction_incorrect(logits, label, 5, name='wrong-top5')
        add_moving_summary(tf.reduce_mean(wrong, name='train-error-top5'))

        wd_cost = regularize_cost('.*/W', l2_regularizer(1e-4), name='l2_regularize_loss')
        # wd_cost_y = regularize_cost('y/W', l2_regularizer(0.1), name='l2_regularize_loss')
        # wd_cost_ynew = regularize_cost('y_new/W', l2_regularizer(0.1), name='l2_regularize_loss')

        add_moving_summary(loss, wd_cost)
        # add_moving_summary(loss, wd_cost_y)
        # add_moving_summary(loss, wd_cost_ynew)
        # add_moving_summary(loss, wd_cost, wd_classifier)
        self.cost = tf.add_n([loss, wd_cost], name='cost')
        # self.cost = tf.add_n([loss, wd_cost, wd_classifier], name='cost')
    ...
    def _get_trainable_variable(self):
	# logger.info("load trainable")
        # exit()
	# print tf.trainable_variables()
	# names = ['y/W:0','y/b:0']
        # self.var_trainable = []
        # for name in names:
        #    var_ = [v for v in tf.global_variables() if v.name == name]
        #    print var_
        #    self.var_trainable.append(var_)
        # print self.var_trainable
        self.var_trainable = None
	logger.info("load trainable {}".format(self.var_trainable))
        return self.var_trainable 
...
def get_data(train_or_test):
    ...
    ds = dataset.customData(datadir, train_or_test, datadir,
    ...
    if isTrain:
        class Resize(imgaug.ImageAugmentor):
            ...
            def _augment(self, img, _):
                ...
                area = h * w
                for _ in range(10):
                    targetArea = self.rng.uniform(0.5, 1.0) * area
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', help='face dataset dir')
